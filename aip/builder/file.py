...
class ProblemFile(FileBuilder):
    ...
    def read_goal(self):
        ...
        goals = []
        for customer in self.data.keys():
            goals.append({"name":"all_delivered","args":[customer],"option":"and"})
...
def PddlBuilder(FileBuilder):
    def __init__(self, raw_data, save_dir, import_dir):
        problem_data = self.process_data(raw_data)

        req_id = problem_data['req_id']
...
if __name__ == "__main__":
    ...
    domain = ProblemFile('test', save_dir, import_dir, {})
    domain.export()